...
import anorm.{ColumnAliaser, RowParser, SQL}
...
abstract class AbstractRepository[M] @Inject()(dbapi: DBApi, databaseName: String = "default")(implicit ec : DatabaseExecutionContext) {
  ...
  def query(queryString: String): Future[Seq[M]] = Future(dbapi.database(databaseName).withConnection { implicit connection =>
